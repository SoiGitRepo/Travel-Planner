...
class PlaceDenseOverlayLayer extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    for (final it in items) {
      ...
      final top = y - 50; // 文字在上，icon 在下
      children.add(
        Positioned(
          left: left,
          top: top,
          child: _DenseItem(placeName: it.place.name, onTap: () async {
            // 选中并相机聚焦
...
class _DenseItem extends StatelessWidget {
  final String placeName;
  final VoidCallback onTap;
  const _DenseItem({required this.placeName, required this.onTap});
